...
export function CatalogCreateForm() {
  ...
  return <form onSubmit={submit}><div className="form-card">
    ...
    <section className="form-section"><h2>External PDF URL</h2><p>Use a public HTTPS PDF URL. RK proxies it server-side, so the source host does not need browser CORS headers.</p><div className="field wide"><label htmlFor="pdfUrl">PDF URL</label><input className="input" id="pdfUrl" name="pdfUrl" type="url" required placeholder="https://example.com/lookbook.pdf" disabled={locked} /><span className="field-hint">The source should be reachable from Render and support range requests where possible.</span></div>
