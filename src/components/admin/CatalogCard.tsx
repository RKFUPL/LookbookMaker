...
import { Archive, Copy, ExternalLink, MoreHorizontal, Pencil, RotateCcw, Send, Trash2, Unlink } from "lucide-react";
...
export function CatalogCard({ catalog, onChanged }: { catalog: CatalogDto; onChanged: () => void }) {
  ...
  const canPreview = catalog.pageCount > 0 && !["downloading", "processing", "failed"].includes(catalog.status);
  return (
    <article className="catalog-card">
      <div className="catalog-cover">
        ...
        <span className={`status-pill ${catalog.status}`}>{["downloading", "processing"].includes(catalog.status) ? `${catalog.processingProgress}% ${catalog.status}` : catalog.status}</span>
      </div>
      <div className="catalog-body">
        <div className="catalog-collection">{catalog.collection}{catalog.season ? ` · ${catalog.season}` : ""}</div>
        ...
        <div className="catalog-actions">
          {isProcessingFailure(catalog.status) ? <button className="btn btn-secondary" disabled={busy} onClick={() => action("/process")}>{catalog.failureCode === "download_failed" ? "Retry" : catalog.failureCode === "storage_missing" ? "Reprocess" : "Retry processing"}</button> : canPreview ? <Link className="btn btn-secondary" href={`/admin/catalogs/${catalog.id}/preview`}>Preview</Link> : <button className="btn btn-secondary" disabled>{catalog.status === "downloading" ? "Downloading" : "Processing"}</button>}
          ...
          <div className="more" ref={root}>
            ...
            {menu && <div className="action-menu">
              ...
              {catalog.sourcePdfUrl && !["downloading", "processing"].includes(catalog.status) && <button type="button" onClick={() => action("/process")}><RotateCcw size={14} /> Reprocess catalog</button>}
