...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!catalog?.sourceKey && !sourceUrl) throw new ApiError(404, "Download is unavailable.");
    ...
    if (!catalog.sourceKey && sourceUrl) return NextResponse.redirect(new URL(sourceUrl), 307);
